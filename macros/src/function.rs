...
pub fn function_impl(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let ItemFn { mut sig, vis, block, attrs } = input;

    let fn_name = sig.ident.clone().to_string();

    let new_sig_name = "remote";
    sig.ident = format_ident!("{}", new_sig_name);

    let arg_names: Vec<_> = sig.inputs
        .iter()
...
pub fn impl_remote_block(
    ...
    quote! {
        ...
        match response {
            Some(RunFunctionResult::Success(success)) => {
                println!("Function succeeded with result: {}", success);
                let result: #output_type = serde_json::from_str(&success)?;
                println!("Parsed result: {:?}", result);
                Ok(result)
            }
            Some(RunFunctionResult::Error(error)) => {
